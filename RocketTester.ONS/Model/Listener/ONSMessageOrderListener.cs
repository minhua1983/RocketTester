    ...
    public class ONSMessageOrderListener : MessageOrderListener
    {
        ...
        public override OrderAction consume(Message value, ConsumeOrderContext context)
        {
            ...
            */
            ...
            if (needToCommit)
            {
                LogHelper.Log("MESSAGE_KEY:" + value.getKey() + ",ons.OrderAction.Success...\n");
                action = ons.OrderAction.Success;
            }
            else
            {
                LogHelper.Log("MESSAGE_KEY:" + value.getKey() + ",ons.OrderAction.Suspend;...\n");
            ...
            try
            {
                if (RT != null)
                {
                    if (topicTag != null)
                    {
                        ...
                        consumeData.Accomplishment = action == OrderAction.Success ? 1 : 0;
